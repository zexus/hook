...
int hook_eglSwapBuffers(char * pcTargetLib)
{
    ...
    if (-1 == nFd) {
        ALOGD("[+] Open taget library error\n");
        nRet = -1;
    }

    void * base_addr = get_module_base(getpid(), pcTargetLib);
    ALOGD("[+] Target Library address = %p\n", base_addr);
    ...
    for (i = 0; i < shnum; i++) {
        ...
        if (shdr.sh_type == SHT_PROGBITS) { // SHT_PROGBITS：表明此节区包含程序定义的信息，其格式和含义都由程序解析
            ...
            if (strcmp(&(string_table[name_idx]), ".got.plt") == 0 || strcmp(&(string_table[name_idx]), ".got") == 0) {
                ...
                ALOGD("[+] Got section start_addr = %lx, section_size = %lx\n", out_addr, out_size);

                for (i = 0; i < out_size; i += 4) {
                    got_item = *(uint32_t *)(out_addr + i);
                    if (got_item  == got_item) {
                        ALOGD("[+] Found eglSwapBuffers in got section\n");
                        got_found = 1;
                        ...
                        break;
                    } else if (got_item == got_item) {
                        ALOGD("Already hooked\n");
                        break;
                    }
                }
                if (got_found)
